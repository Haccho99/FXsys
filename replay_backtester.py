...
class OHLCReplayJob:
    def __init__(self, pair: str, start_date: str, end_date: str, initial_balance: float = 1_000_000.0):
        ...
        self.broker = VirtualBroker(initial_balance=initial_balance, is_backtest=True)
    ...
    async def run(self):
        ...
        for i, row in enumerate(sim_df.iter_rows(named=True)):
            ...
            # 決済されたポジションの記録
            if hasattr(self.broker, 'closed_positions_history') and self.broker.closed_positions_history:
                for log in self.broker.closed_positions_history:
                    log["profit_jpy"] = log.get("profit_amount", 0)
                    self.trade_stats.append(log)
                self.broker.closed_positions_history.clear()
        ...
        return self.broker.balance, self.broker.positions, self.trade_stats, self.trade_stats

async def main():
    import argparse
    parser = argparse.ArgumentParser(description="OHLC Replay Backtester")
    parser.add_argument("--start", type=str, default="2025-07-01 00:00:00", help="Simulation start date (YYYY-MM-DD HH:MM:SS)")
    parser.add_argument("--end", type=str, default="2026-06-30 00:00:00", help="Simulation end date (YYYY-MM-DD HH:MM:SS)")
    args = parser.parse_args()
    
    SIM_START_DATE = args.start
    SIM_END_DATE   = args.end
    TARGET_PAIRS   = cfg.get_sync("trading.symbols", ["USD_JPY", "EUR_JPY", "GBP_JPY", "AUD_JPY", "AUD_USD", "EUR_USD", "GBP_USD"])
    ...
    for i, pair in enumerate(TARGET_PAIRS):
        if r:
            try:
                ...
                r.set("job_progress:backtest", str(progress))
                r.set("job_phase:backtest", f"【{pair}】 検証中... ({i+1}/{total_pairs})")
            except Exception:
                pass
    ...
    if r:
        try:
            r.set("job_progress:backtest", "1.0")
            r.set("job_phase:backtest", "全ペア検証完了")
        except Exception:
            pass
